#################################################################################
# This file is used to generate Figures 2 and 3 in Section 4.1.
# This code compares the proposed method (rome) with two benchmarks (hqreg and 
# ILAMM) under various data-generating processes in terms of RMSE and CPU time.
#################################################################################
get_sim <- function(n,p,type,iter_max){
  ...
  run_sim <- foreach(iter = 1:iter_max,
                     .errorhandling = 'stop',
                     .packages = c("Matrix","mvtnorm","rome","hqreg","ILAMM"),
  ...
  stopCluster(cl)
  save(run_sim,file=paste0("Benchmark_comparison_type_",type,"_p",p,"_n",n,".RData"))
}
...
for (i in 1:3){
  ...
  for (j in 1:3){
    ...
    for (k in 1:4){
      ...
      cat("The current iteration is p:",p,"n:",n,"type is",type,"\n")
      load(paste0("Benchmark_comparison_type_",type,"_p",p,"_n",n,".RData"))
